...
WORK_DIR=$(mktemp -d -p "$SCRATCH_BASE" "tps_traj_${TRAJ_PAD}_${JOB_TAG}_XXXXXX")
...
cp "$SOURCE_DIR/NN-tps.tar.xz" "$WORK_DIR/"

cd "$WORK_DIR"
tar -xf "NN-tps.tar.xz"
...
cp -a tpsagg.log tpsagg.md.energies tpsagg.md.xyz tpsagg.md.velo "$RESULT_DIR/" 2>/dev/null || true
cp -a tpsagg.sh.energies tpsagg.sh.xyz tpsagg.sh.velo "$RESULT_DIR/" 2>/dev/null || true
