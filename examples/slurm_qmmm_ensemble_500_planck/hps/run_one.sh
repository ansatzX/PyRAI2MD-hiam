...
WORK_DIR=$(mktemp -d -p "$SCRATCH_BASE" "hps_traj_${TRAJ_PAD}_${JOB_TAG}_XXXXXX")
...
cp "$SOURCE_DIR/NN-hps.tar.xz" "$WORK_DIR/"

cd "$WORK_DIR"
tar -xf "NN-hps.tar.xz"
...
cp -a hpsagg.log hpsagg.md.energies hpsagg.md.xyz hpsagg.md.velo "$RESULT_DIR/" 2>/dev/null || true
cp -a hpsagg.sh.energies hpsagg.sh.xyz hpsagg.sh.velo "$RESULT_DIR/" 2>/dev/null || true
